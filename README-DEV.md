...
The stack is containerized via `docker-compose.yml` and bound to the `50100-50150` range to avoid host conflicts:
- **`50100` - MongoDB**: Core database.
- **`50101` - Mongo Express**: Web-based GUI for rapid database inspection.
- **`50102` - FastAPI**: The REST API endpoints.
...
1. Start the stack.
2. Navigate to **http://localhost:50102/docs** to see the **interactive Swagger UI** generated live from the Python endpoints. Here you can execute trial queries against the MongoDB container without needing Postman.
3. If you want the raw OpenAPI schema to generate client SDKs, navigate to **http://localhost:50102/openapi.json**.
...
- **Programmatic / Compass URI**: `mongodb://admin:secretpassword@localhost:50100/`
- **Web UI (Mongo Express)**: Open `http://localhost:50101` in a browser. Standard HTTP Basic Auth will prompt for `admin` / `secretpassword`.
