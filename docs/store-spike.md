...
| 4 | **Loyalty prices** | The loyalty offer category (14288) returns 0 items anonymously, and `loyaltyPrice` is omitted. **But `packageInfo.loyaltyUnitPrice` is sent anonymously** for every product with the `flag_loyalty_price` flag (380/380 on 2026-09-17): card price = discounted unit price × pack size (per-kg price for loose items), matching shelf prices. Árfigyelő reports 106 Auchan `LOYALTY` prices for cross-checking |
