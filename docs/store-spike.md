...
| 4 | **Loyalty prices** | The loyalty offer category (14288) returns 0 items anonymously, and `loyaltyPrice` is omitted. `packageInfo.loyaltyUnitPrice` is sent anonymously for every product with the `flag_loyalty_price` flag (380/380 on 2026-09-17). Card price = discounted unit price × pack size, or regular × (1 − discount %) when that rounding error is smaller (many-piece packs). **Validation against Árfigyelő (2026-09-17):** of 106 Auchan `LOYALTY` prices, 41 barcodes are in the crawl; 5 carry the anonymous card unit price and 4 match exactly (the fifth disagrees within Árfigyelő). The other 36 have a card price in Árfigyelő, mostly exactly 30 % off, with **no card data in anonymous responses**. Anonymous card prices are therefore incomplete |
